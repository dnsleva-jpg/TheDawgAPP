...
import { BrainRecoveryChart } from '../components/rewire/BrainRecoveryChart';
...
function ProgressRing({ day, totalDays }: { day: number; totalDays: number }) {
  ...
  // Pure View ring: background ring + overlay to clip progress
  // Simple approach: show the day number in a styled circle
  return (
    <View style={{
      width: size,
      height: size,
      ...
      {/* Progress arc — rendered as a colored border overlay */}
      {/* Top-right quadrant */}
      {pct > 0 && (
        <View style={{
          position: 'absolute',
          ...
          borderTopColor: pct >= 0.25 ? DS_COLORS.coral : DS_COLORS.coral,
          borderRightColor: pct >= 0.5 ? DS_COLORS.coral : 'transparent',
          borderBottomColor: pct >= 0.75 ? DS_COLORS.coral : 'transparent',
          borderLeftColor: pct >= 1 ? DS_COLORS.coral : 'transparent',
          transform: [{ rotate: '-90deg' }],
        }} />
      )}
      <Text style={{ fontFamily: FONTS.display, fontSize: 36, color: DS_COLORS.textPrimary, lineHeight: 38 }}>
        {day}
      </Text>
      <Text style={{ fontFamily: FONTS.mono, fontSize: 10, color: DS_COLORS.textMuted, letterSpacing: 1 }}>
        OF {totalDays}
      </Text>
    </View>
  );
...
export function RewireScreen({ isPro = false, onPaywall }: RewireScreenProps) {
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      >
        ...
        <View style={styles.ringContainer}>
          <ProgressRing day={rewireDay} totalDays={90} />
          <View style={styles.ringMeta}>
            <Text style={styles.ringPhase}>
              {rewireDay <= 14 ? 'AWARENESS' : rewireDay <= 30 ? 'REDUCTION' : rewireDay <= 60 ? 'TRAINING' : 'MASTERY'}
            </Text>
            <Text style={styles.ringProgress}>
              {Math.round((rewireDay / 90) * 100)}% complete
            </Text>
          </View>
        </View>
