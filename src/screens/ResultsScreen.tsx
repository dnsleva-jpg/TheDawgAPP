...
export function ResultsScreen({ completedSeconds, videoUri, stillnessPercent = 0, blinksCount = 0, scoringResults, protectionLevel = 'easy', onGoHome }: ResultsScreenProps) {
  ...
  const loadStats = async () => {
    try {
      const [sessions, onboarding] = await Promise.all([
        getSessions(),
        getOnboardingData(),
      ]);
      ...
      // Baseline delta
      if (onboarding.baselineScore != null) {
        setBaselineScore(onboarding.baselineScore);
      }
      // First session = only 1 completed session total (the one just saved)
  ...
  const handleShareToStories = async () => {
    ...
    try {
      ...
      const storiesUrl = `instagram-stories://share?source_application=com.dustinsleva.rawdawgapp&backgroundImage=${encodeURIComponent(base64)}`;
      const canOpen = await Linking.canOpenURL('instagram-stories://share');
      if (canOpen) {
        await Linking.openURL(storiesUrl);
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      } else {
        // Fallback: save to camera roll and let user post manually
        const { status } = await MediaLibrary.requestPermissionsAsync();
        if (status === 'granted') {
          await MediaLibrary.createAssetAsync(uri);
        }
        Alert.alert(
          'Instagram Not Found',
          'Card saved to your camera roll. Open Instagram Stories and add it from your gallery.',
        );
      }
    } catch (error: any) {
      Alert.alert('Share Failed', error?.message ?? 'Could not share to Instagram Stories.');
    } finally {
      setIsSharingCard(false);
    }
  };
        ...
        {scoringResults ? (
          <>
            ...
            <View style={styles.heroContainer}>
              ...
              {/* Baseline Delta */}
              {isFirstSession ? (
                <Text style={styles.baselineFirstSession}>Your journey starts here.</Text>
              ) : baselineScore != null && scoringResults.dawgScore != null ? (
                (() => {
                  const delta = Math.round(scoringResults.dawgScore - baselineScore);
                  if (delta > 0) {
                    return <Text style={styles.baselineDeltaPositive}>↑ {delta} points from your Day 0 baseline</Text>;
                  } else if (delta < 0) {
                    return <Text style={styles.baselineDeltaNegative}>↓ {Math.abs(delta)} points from your Day 0 baseline</Text>;
                  } else {
                    return <Text style={styles.baselineDeltaNeutral}>— Same as your Day 0 baseline</Text>;
                  }
                })()
              ) : null}
            </View>
            ...
            <View style={styles.subScoreCard}>
              <SubScoreBar label="STILLNESS" value={scoringResults.stillnessScore} color="#D4820A" />
        ...
        <View style={styles.shareCardOverlay}>
          ...
          >
            ...
            <View style={styles.shareCardActions}>
              <TouchableOpacity
                style={styles.igStoriesButton}
                onPress={handleShareToStories}
                activeOpacity={0.8}
                disabled={isSharingCard}
              >
                <Text style={styles.igStoriesButtonText}>SHARE TO IG STORIES</Text>
              </TouchableOpacity>
