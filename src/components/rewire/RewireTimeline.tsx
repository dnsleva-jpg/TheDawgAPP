...
export function RewireTimeline({ sessions, startDate, onCurrentDayYOffset }: Props) {
  ...
  // How many milestones/segments to actually render (completed + next target only)
  const visibleMsCount = nextMsIdx === -1 ? ALL_MS.length : nextMsIdx + 1;
  const visibleSegCount = visibleMsCount - 1;
  const visibleTotalH = visibleMsCount > 0
    ? msPositions[visibleMsCount - 1].y + NODE / 2 + 100
    : totalH;
  ...
  return (
    <View style={[st.root, { height: visibleTotalH }]}>
      <Text style={st.startDate}>STARTED {dateStr.toUpperCase()}</Text>

      {/* ── Path Lines ── */}
      {pathSegs.map((s, i) => {
        if (s.segIdx >= visibleSegCount) return null;
      ...
      {SEGMENTS.map((seg, i) => {
        if (i >= visibleSegCount) return null;
      ...
      {todaySegIdx >= 0 && todaySegIdx < visibleSegCount && !expandedSegs[todaySegIdx] && (() => {
      ...
      {/* ── Milestone Nodes ── */}
      {ALL_MS.map((ms, i) => {
        if (i >= visibleMsCount) return null;
      ...
      {/* Finish line — only show when all milestones are visible */}
      {nextMsIdx === -1 && (
