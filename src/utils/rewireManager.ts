...
const START_DATE_KEY = 'dawg_start_date';
...
export function computeTrends(sessions: Session[]): TrendData {
  const scored = sessions
    .filter((s) => s.dawgScore != null && s.stillnessPercent != null)
...
export function computeBrainVsAverage(sessions: Session[]): BrainComparison {
  const scored = sessions.filter(
    (s) => s.dawgScore != null && s.stillnessPercent != null
  );
...
export type DayStatus = 'completed' | 'current' | 'currentCompleted' | 'missed' | 'unlocked' | 'future';

export interface TimelineDayStatus {
  dayNumber: number; // 1-90
  date: string; // YYYY-MM-DD
  status: DayStatus;
  revisitable?: boolean; // true for missed days that can be revisited
}
...
export function getTimelineDayStatuses(
  ...
  for (let i = 0; i < 90; i++) {
    ...
    if (dateStr === today) {
      ...
      if (!isDone) revisitable = true;
    } else if (dayNumber <= calendarDay + 3) {
      // Next 3 days after current are "unlocked" (visible, previewable)
      status = 'unlocked';
    } else {
      status = 'future';
    }

    result.push({ dayNumber, date: dateStr, status, revisitable });
  }
...
const DAILY_CHALLENGES_KEY = 'dawg_daily_challenges';
...
const CELEBRATED_KEY = 'dawg_celebrated_milestones';
